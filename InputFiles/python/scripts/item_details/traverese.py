...
api2_file    = open("InputFiles/python/output/json/api_items_price2.json","r")
api2_json   = json.load(api2_file)

def find_recepie(dict,i=0):
   if len(dict)==0:return print()
   print("-> "+dict["id"],end='')
   if len(dict["recepie"]) == 0:return print()
   if api2_json.get(dict["recepie"][i]) is None:return
   find_recepie(api2_json[dict["recepie"][i]],i)
   i=i+1
   if len(dict["recepie"]) <= i:return
   find_recepie(api2_json[dict["recepie"][i]])
...
for i in api2_json:
    ...
    while(len(recepie) != j and api2_json.get(recepie[j]) is not None):
        find_recepie(api2_json[recepie[j]])
        j=j+1
print()
t2 = time.time()
print(t2-t1)
